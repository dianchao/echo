...
public class EchoServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        //当客户端连接成功后进行此方法的调用。明确可以给客户端发送一些信息
        byte[] data = "【服务器激活信息】连接通道已经创建，服务器开始进行响应交互。".getBytes();
        //NIO是基于缓存的操作，所以Netty也提供了一系列的缓存类（封装了NIO中的Buffer）
        ByteBuf buf = Unpooled.buffer(data.length); //Netty自己定义的缓存类
        buf.writeBytes(data); //将数据写入到缓存中
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        try {
            ...
            //对于客户端发送来的数据信息，由于没有进行指定数据类型，所以统一按照Object进行接收
            ByteBuf buf = (ByteBuf) msg; //默认情况下的类型就是ByteBuf类型
            //在进行数据类型转换的过程中还可以进行编码指定（NIO的封装）
            String inputData = buf.toString(CharsetUtil.UTF_8); //将字节缓冲区的内容转换为字符串
            ...
            byte[] data = echoData.getBytes();//将当前字符串转变为字节数组
